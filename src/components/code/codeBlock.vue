...
export default {
    ...
    computed: {
        ...
        textFieldCodes() {
            let code = this.code.map(function (field, index) {
                ...
                var spacingForTextFieldProp =
                    field.type === 'Date' ? doublespace + doublespace + doublespace : doublespace + doublespace;
                ...
                var rule =
                    field.required || textLengthRule != ''
                        ...
                        : '';
                //label
                var label = nextLine + spacingForTextFieldProp + 'label=' + doubleQuotes + field.title + doubleQuotes;
                //rouned
                var textFieldClass = field.rounded
                    ? nextLine + spacingForTextFieldProp + 'class=' + doubleQuotes + 'rounded-xl' + doubleQuotes
                    : '';
                //dense
                var denseProp = field.dense
                    ? nextLine + spacingForTextFieldProp + ':dense=' + doubleQuotes + field.dense + doubleQuotes
                    : '';
                //outlined
                var outlinedProp = field.outlined
                    ? nextLine + spacingForTextFieldProp + ':outlined=' + doubleQuotes + field.outlined + doubleQuotes
                    : '';
                //END OF COMMON THINGS FOR ALL TEXT FIELD
                //Date template
                if (field.type === 'Date') {
                    let minValue = field.minCurrentDay ? ' :min=' + doubleQuotes + 'currentDay' + doubleQuotes : '';
                    var dateComponent =
                        nextLine +
                        doublespace +
                        space +
                ...
                if (field.type !== 'Date') {
                    ...
                    var hint =
                        field.type === 'Password'
                            ? '\n        hint=' + doubleQuotes + 'At least 8 characters' + doubleQuotes
                            : '';
                    var appendicon =
                        field.type === 'Password'
                            ? '\n        :append-icon=' +
                    ...
                    var value =
                        '\n        v-model' + numberVModel + '=' + doubleQuotes + 'field' + (index + 1) + doubleQuotes;
        ...
        dataPart() {
            ...
            const emailFieldFound = this.code.some((field) => field.type === 'E-Mail');
            const passwordFieldFound = this.code.some((field) => field.type === 'Password');
            const numberFieldFound = this.code.some((field) => field.type === 'Number');
            const currentDayValidationFound = this.code.some((field) => field.minCurrentDay);
            ...
            let requiredRules = requiredFound
                ? '\n      requiredRules: ' + "(v) => !!v || 'This field is required.'" + comma
                : '';
            ...
            let passwordRules = passwordFieldFound
                ? "\n      passwordRules: (v) => (v && v.length >= 8) || 'Minimum 8 characters.'" + comma
                : '';

            let code = this.code.map(function (field, index) {
                let showDatePicker =
                    field.type === 'Date' ? nextLine + space + 'showDatePicker' + (index + 1) + ' : false' + comma : '';
                let showPass =
                    field.type === 'Password' ? '\n      showPassOnField' + (index + 1) + ' : false' + comma : '';
                let value =
                    field.type === 'Date'
                        ? 'date' + (index + 1) + ' : ' + doubleQuotes + doubleQuotes + comma
                        : 'field' + (index + 1) + ' : ' + doubleQuotes + doubleQuotes + comma;
                let fieldValue = nextLine + space + value;
                let data = fieldValue + showDatePicker + showPass;
                return data;
            });
            return code.join('') + passwordRules + emailRules + requiredRules + numberRules + currentDay;
        },
        fullCode() {
            return (
                template[0] +
