// Vuetify 4 code generator
// Vuetify 4 is similar to Vuetify 3 but with Vue 3.5+ features and some additional components
...
const generateDataProperties = (fields) => {
  ...
  fields.forEach((field, index) => {
    if (field.type === 'Date') {
      props.push({ name: `date${index + 1}`, value: 'null', type: 'Date | null' });
...
const generateTemplate = (fields) => {
  ...
  const fieldComponents = fields
    .map((field, index) => generateField(field, index, 4, 3))
    .join('\n');
...
// Composition API with script setup (preferred for Vuetify 4)
...
export const generateVuetify4Code = (fields, options = {}) => {
...
export default generateVuetify4Code;
